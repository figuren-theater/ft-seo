...
function persist_current_post( bool $disable_autogeneration, $post_id) : bool {
	...
	if ( ! post_type_supports( 
		...
		get_post_type( currentPost::init()->get_id() ), 
...
function pre_option_sharing_image_templates( $option ) : array|bool {
    ...
    // 
    $template = __get_featured_image($template);
    
    // 
    $template = __get_shortlink($template);
...
function __get_featured_image( array $template ) : array {
	// 0.
	if ( isset($template['image']) && esc_url( $template['image'] ))
		return $template;
...
function __get_shortlink( array $template ) : array {
	// prepare url
	$url = null;	
